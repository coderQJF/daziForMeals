...
const recipes = ref<KitchenRecipe[]>([
  {
    id: 1003,
    name: '妈妈的红烧肉',
    description: '软糯入味，经典下饭菜，记忆中的味道。',
    cookTime: 60,
    servings: 2,
    cover: '/static/images/category/takeout/hot-pot-cover.jpg',
    isFavorite: true,
    cooked: true,
  },
  {
    id: 1004,
    name: '番茄牛肉汤',
    description: '酸甜开胃，汤鲜味美，营养又暖心。',
    cookTime: 40,
    servings: 2,
    cover: '/static/images/category/cooking/home-style-cover.jpg',
    isFavorite: true,
    cooked: false,
  },
  {
    id: 1005,
    name: '青椒炒牛肉',
    description: '鲜嫩滑口，家常快手，超级下饭。',
    cookTime: 20,
    servings: 2,
    cover: '/static/images/category/cooking/quick-cover.jpg',
    isFavorite: true,
    cooked: false,
  },
])
...
const visibleRecipes = computed(() => {
  return activeTab.value === 'favorite'
    ? recipes.value.filter(recipe => recipe.isFavorite)
    : recipes.value.filter(recipe => recipe.cooked)
})
...
function toggleFavorite(recipe: KitchenRecipe) {
  ...
  uni.showToast({
    title: recipe.isFavorite ? '已加入收藏' : '已取消收藏',
    icon: 'none',
  })
}
...
function openRecipeMenu(recipe: KitchenRecipe) {
  uni.showActionSheet({
    itemList: [recipe.isFavorite ? '取消收藏' : '加入收藏', '添加到饮食计划', '标记为做过'],
    success: ({ tapIndex }) => {
      if (tapIndex === 0) {
  ...
  <view class="favorite-page">
    ...
    <view class="page-header">
      ...
      <button class="page-header__bell" aria-label="查看消息" @click="showNotice">
        <image
          class="page-header__bell-icon"
          src="/static/images/favorite/notification-bell.png"
          mode="aspectFit"
        />
      </button>
    </view>
